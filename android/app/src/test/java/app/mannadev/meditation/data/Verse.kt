...
class VerseTest {
    ...
    fun `fromDto standard input`() {
        ...
        assertEquals(expectedSermon.title, actualVerse.title)
        assertEquals(expectedSermon.bookName, actualVerse.bookName)
        assertArrayEquals(
            expectedSermon.verses.toTypedArray(),
    ...
    fun `fromDto handles content without '본문 colon' prefix gracefully`() {
        ...
        val expectedSermon = Sermon(
            ...
            bookName = "시편 23:1-2",
            title = sermonDto.title
        )
        val actualVerse = VerseParser.verseDtoToVerse(sermonDto)
        assertEquals(expectedSermon, actualVerse)
    }
    ...
    fun `fromDto book name with leading trailing spaces`() {
        ...
        val exception = runCatching { VerseParser.verseDtoToVerse(sermonDto) }.exceptionOrNull()
